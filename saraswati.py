...
class Swarm:
    def __init__(self, agents: list = None, max_loops: int = 1):
        # Avoid mutable default argument
        self.agents = agents if agents is not None else [counselor_agent, buddy_agent]
        self.max_loops = max_loops
        self.conversation = Conversation()

    def step(self, task: str) -> str:
        """
        Process a single interaction with the swarm.
        
        Args:
            task (str): The user's input/query
            
        Returns:
            str: Conversation history as string
...
def main():
    """Main entry point for the SARASWATI system."""
    # Setup logging
    logging.basicConfig(
        level=logging.INFO,
        format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
        handlers=[
            logging.FileHandler('saraswati.log'),
    ...
    try:
        ...
        print("Welcome to SARASWATI College Guidance System!")
        ...
        while True:
            ...
            if user_input.lower() in ['quit', 'exit', 'q']:
                print("Thank you for using SARASWATI. Goodbye!")
                break
            ...
            try:
                response = swarm.step(user_input)
                print("\nResponse:", response)
            except Exception as e:
